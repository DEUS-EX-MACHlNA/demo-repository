...
def _truncate_at_sentence(text: str, max_chars: int = 80) -> str:
...
def quality_gate(text: str) -> Tuple[str, List[str]]:
    ...
    if len(result) > 80:
        result = _truncate_at_sentence(result, max_chars=80)
...
if __name__ == "__main__":
    ...
    # 글리치 후처리 데모
    print("\n--- 2단계: 통합 파이프라인 (품질 검증 + 글리치) ---")
    clean_outputs = [
        t for t in test_outputs
        if len(t.strip()) > 5 and not _is_character_break(t)
    ]
    for level in [1, 2, 3]:
        print(f"\n  glitch_level = {level}")
        for i, original in enumerate(clean_outputs):
            processed = postprocess(original, glitch_level=level, seed=i * 100 + level)
