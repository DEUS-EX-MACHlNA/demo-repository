...
def _ensure_str(value: Any) -> str:
    """값이 리스트면 join, 문자열이면 그대로, 그 외 빈 문자열 반환."""
...
def format_persona(persona: dict[str, Any]) -> str:
    ...
    values = _ensure_str(persona.get("values", ""))
    taboos = _ensure_str(persona.get("taboos", ""))
    relationships = _ensure_str(persona.get("relationships", ""))
    parts = []
    if values:
    ...
    if relationships:
        parts.append(f"관계: {relationships}")
...
def format_emotion(stats: dict[str, int]) -> str:
    ...
    Args:
        stats: NPC 스탯 Dict (예: {"affection": 50, "humanity": 0})
    """
    if not stats:
        return "(스탯 정보 없음)"
...
def parse_stat_changes_text(text: str, stat_names: list[str] | None = None) -> dict[str, int]:
    ...
    Args:
        text: LLM 응답 텍스트
        stat_names: 파싱할 스탯 이름 리스트 (예: ["affection", "humanity"])
                    None이면 텍스트에서 "이름: 숫자" 패턴을 자동 감지

    예상 포맷: "affection: +1, humanity: -1"
    """
    result: dict[str, int] = {}
    if stat_names:
